...
public class PdfReportGenerator
{
    ...
    private void ComposeConfigurationPage(IContainer container, Project project, ConfigurationResult result, int configNumber)
    {
        ...
        container.PaddingVertical(5).Column(column =>
        {
            ...
            column.Item().Table(table =>
            {
                ...
                // Row 3: Parcel number (optional)
                if (!string.IsNullOrEmpty(project.ParcelNumber))
                {
                    table.Cell().Padding(2).Text(s.ParcelNumber + ":").FontSize(9).Bold();
                    table.Cell().Padding(2).Text(project.ParcelNumber).FontSize(9);
                    table.Cell(); table.Cell();
                }
            });
        ...
        container.Border(1).BorderColor(borderColor).Padding(10).Column(column =>
        {
            ...
            if (result.BandResults.Count > 0)
            {
                column.Item().PaddingTop(8).Table(table =>
                {
                    ...
                    // Header row: one column per band (band name); the exact frequency is the first data row
                    table.Header(header =>
                    {
                        ...
                        foreach (var band in result.BandResults)
                        {
                            header.Cell().Background(headerBg).Padding(3).AlignRight()
                                .Text(MasterDataStore.GetBandName(band.FrequencyMHz)).Style(headerStyle);
                        }
                    });
    ...
    private void ComposeFooter(IContainer container)
    {
        container.AlignCenter().Text(text =>
        {
            ...
            text.Span($"Generated by Swiss NIS Calculator {AppInfo.Version} | ");
            text.Span("Page ");
